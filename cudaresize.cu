...
void transformKernel(float *output, float wr, float hr, float dr, 
    ...
    if (x < width && y < height && z < depth)
        output[z*width*height + y*width + x] = cubicTex3D(texRef, u, v, w);
}

template<class T> 
void interpolate_template(
        const T *input_array, T *output_array, unsigned int width, unsigned int height, unsigned int depth,
        unsigned int new_width, unsigned int new_height, unsigned int new_depth, bool prefilter
    ) {
...
extern "C" void interpolate(
    ...
    interpolate_template(input_array, output_array, width, height, depth, new_width, new_height, new_depth, prefilter);
}
